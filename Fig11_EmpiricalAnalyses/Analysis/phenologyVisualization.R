...
#' @export
makePosteriorPredictivePlot = function(stanResult, responseData, responseVariableName="DOY", targetCovariateName, onsetCovariateData, durationCovariateData, n_samples=100, n_draws=100, n_hist=10000, n_bin=100, resolution=50, slice = 0.25, N=1000, minResponse=0, maxResponse=365, adjustYLim=T, smooth=c("GAM", "LOESS", "SPLINE", "NONE"), keepSigma=FALSE, makeHistograms=FALSE) {
	...
	#get draws
	posterior_samples  =  posterior::as_draws_df(stanResult$result$draws())
	...
	if(nColO > 1) {
		...
		UO  =  copula::pobs(as.matrix(onsetCovariateData))  # Pseudo-observations (uniform marginals)
		copO  =  copula::normalCopula(dim = ncol(UO), dispstr = "un")
		fitO  =  copula::fitCopula(copO, UO, method = "ml")

		UD  =  copula::pobs(as.matrix(durationCovariateData))  # Pseudo-observations (uniform marginals)
		copD  =  copula::normalCopula(dim = ncol(UD), dispstr = "un")
		fitD  =  copula::fitCopula(copD, UD, method = "ml")
	}
	...
	for (x in target_vals) {
		...
		results  =  dplyr::bind_rows(results, pred_summary)
	}
	...
	#make shift graphs in three panels
if(!makeHistograms) {
	...
	filtered_results  =  results %>%
		...
		dplyr::filter(dplyr::row_number() < dplyr::n()) %>%  # keep all but last row per group
		dplyr::ungroup()
	...
	if(is.na(smooth)) {
		...
		filtered_results  =  filtered_results %>%
			...
			dplyr::group_modify(~ {
									...
									x_target = .x$x_target,
									series = .x$series,
									mean  = predict(mgcv::gam(mean  ~ s(x_target), data = .x, method = "REML")),
									lower = predict(mgcv::gam(lower ~ s(x_target), data = .x, method = "REML")),
									upper = predict(mgcv::gam(upper ~ s(x_target), data = .x, method = "REML"))
							 )
			}) %>%
			dplyr::ungroup()
	}
		...
		filtered_results  =  filtered_results %>%
			...
			dplyr::group_modify(~ {
									...
									x_target = .x$x_target,
									series = .x$series,
									mean  = predict(lm(mean  ~ splines::bs(x_target, df = 5), data = .x)),
									lower = predict(lm(lower ~ splines::bs(x_target, df = 5), data = .x)),
									upper = predict(lm(upper ~ splines::bs(x_target, df = 5), data = .x))
							 )
			}) %>%
			dplyr::ungroup()
	}
	...
	figure = cowplot::plot_grid(pPPP, slices, nrow = 1, ncol = 2, labels = c("A"), label_x = 0, label_y = 0.1, rel_widths = c(2, 1))
...
posterior_predictive_graphic = function(observed_data, filtered_results,targetCovariateName,responseVariableName,legend=TRUE, timeSlice1, timeSlice2, adjustYLim = T, minResponse=0, maxResponse=365) {
	...
	pPPP = ggplot2::ggplot() +
		...
		ggplot2::geom_ribbon(
					...
					ggplot2::aes(x = x_target, ymin = lower, ymax = upper, fill = series),
					alpha = 0.2,
					color = NA
					) +
		...
		ggplot2::geom_line(
				  data = filtered_results,
				  ggplot2::aes(x = x_target, y = mean, color = series),
				  size = 0.5
				  ) +
		...
		ggplot2::geom_vline(xintercept = c(timeSlice1, timeSlice2), linetype = "dashed", color = "black") +
		...
		ggplot2::theme(
			  ...
			  legend.background = ggplot2::element_rect(fill = "white", color=NA )  # opaque bg
		)
		if(!legend) {
			pPPP = pPPP + ggplot2::theme(legend.position = "none")
		}
		if(adjustYLim) {
			print(ylim)
			pPPP = pPPP + ggplot2::ylim(ylim[1], ylim[2])
		}
		else {
			pPPP = pPPP + ggplot2::coord_cartesian(ylim = c(minResponse, maxResponse))
		}
		return(pPPP)
}
...
# Plot with density curves and arrow at ymid
#ggplot(df, aes(x = value)) +
	#geom_density(data = df %>% filter(timepoint == "T1"),
               #aes(x = value),
			   #from = min_x, to = max_x,
               #fill = col, color = col, alpha = 0.1, linewidth = 0.5) +
  #geom_density(data = df %>% filter(timepoint == "T2"),
               #aes(x = value),
			   #from = min_x, to = max_x,
               #fill = col, color = col, alpha = 0.1, linewidth = 0.5) +
...
makeSimulatedAndTheoreticalOverlayGraph= function(mu_O, mu_C, sigma, n_hist, N, minResponse, maxResponse, responseVariableName="DOY", xlim=c(0,365), legend=FALSE, nBin=50, includeHistograms=FALSE) {
	...
	if(includeHistograms) {
		...
		p = ggplot2::ggplot(df, ggplot2::aes(x = value, fill = dist, color = dist)) +
			ggplot2::geom_histogram(ggplot2::aes(y = ..density..), position = "identity", alpha = 0.3, bins = nBin) +
			ggplot2::scale_fill_manual(
							  values = c(
										 "Ok1" = "yellow",
										 "O"   = "red",
			...
			ggplot2::stat_function(fun = dOk1, args = list(N=N,mu_O=mu_O, sigma_O=sigma, minResponse=minResponse, maxResponse=maxResponse, type="GP"), ggplot2::aes(color = "Ok1"), size = 1) +
			ggplot2::stat_function(fun = dO, args = list(mu_O=mu_O, sigma_O=sigma, minResponse=minResponse, maxResponse=maxResponse, type="GP"), ggplot2::aes(color = "O"), size = 1) +
			ggplot2::stat_function(fun = dT, args = list(mu_O=mu_O, sigma_O=sigma, mu_D=mu_D, minResponse=minResponse, maxResponse=maxResponse, type="GP"), ggplot2::aes(color = "T"), size = 1) +
			ggplot2::stat_function(fun = dC, args = list(mu_O=mu_O, sigma_O=sigma, mu_D=mu_D, minResponse=minResponse, maxResponse=maxResponse, type="GP"), ggplot2::aes(color = "C"), size = 1) +
			ggplot2::stat_function(fun = dCkN, args = list(N=N,mu_O=mu_O,sigma_O=sigma, mu_D=mu_D, minResponse=minResponse, maxResponse=maxResponse, type="GP"), ggplot2::aes(color = "CkN"), size = 1) +
			ggplot2::xlim(xlim[1], xlim[2])+

			ggplot2::geom_vline(xintercept = xIs, color = "black") +
		...
		df  =  dplyr::tibble(x = x, Ok1_d,O_d,T_d,C_d,CkN_d) %>%
			tidyr::pivot_longer(cols = c(Ok1_d,O_d,T_d,C_d,CkN_d), names_to = "curve", values_to = "y")

		p = ggplot2::ggplot(df, ggplot2::aes(x = x, y = y, fill = curve)) +
			ggplot2::geom_area(alpha = 0.4, position = "identity") +
			ggplot2::geom_line(ggplot2::aes(color = curve), size = 0.5) +
